...
export default function CustomerDetailScreen() {
  ...
  const handleChangePassword = async () => {
    if (!customer.user_id) {
      Alert.alert('Lỗi', 'Khách hàng chưa có tài khoản đăng nhập')
      return
    }
    ...
    Alert.prompt(
      'Đổi mật khẩu',
      'Nhập mật khẩu mới cho khách hàng (tối thiểu 6 ký tự):',
      [
        {
          text: 'Hủy',
          style: 'cancel'
        },
        {
          ...
          onPress: async (password?: string) => {
            ...
            try {
              ...
              // Note: Changing password requires admin API
              // For now, we'll show a message
              Alert.alert(
                'Thông báo',
                'Để đổi mật khẩu, vui lòng sử dụng chức năng "Quên mật khẩu" trong trang đăng nhập, hoặc liên hệ admin hệ thống.',
                [{ text: 'OK' }]
              )
            } catch (error: any) {
