...
function Satellite({ brand, index, total }: SatelliteProps) {
  ...
  // Calculate orbital parameters for atom-like orbits
  const orbitRadius = 4;
  const orbitSpeed = 0.3 + (index % 3) * 0.1;
  const phaseOffset = (index * Math.PI * 2) / total;
  
  // Different orbital planes like electrons in an atom
  // Create 3 different orbital planes
  const orbitPlane = index % 3;
  const inclinations = [
    { x: Math.PI / 2, y: 0, z: 0 },           // Vertical orbit (XY plane)
    { x: Math.PI / 3, y: Math.PI / 4, z: 0 }, // Tilted orbit 1
    { x: Math.PI / 6, y: -Math.PI / 4, z: 0 } // Tilted orbit 2
  ];
  
  const inclination = inclinations[orbitPlane];
  
  // Trail colors based on orbit plane
  ...
  useFrame(({ clock }) => {
    ...
    // Apply rotation to create different orbital planes
    ...
    // Keep logo facing camera (billboard effect)
    meshRef.current.lookAt(0, 0, 10);
  });

  return (
    <>
      {/* Trail line with glow effect */}
      <Trail positions={trailPositions.current} color={trailColor} />
      
      <group ref={meshRef}>
        {/* Satellite sphere with glow */}
        <mesh>
        ...
        {/* Logo HTML overlay - always facing camera */}
        <Html
          center
          distanceFactor={8}
          style={{
...
function OrbitRings() {
  const rings = useMemo(() => {
    ...
    // Create 3 orbital rings at different angles like atom orbitals
    const orbitPlanes = [
      { x: Math.PI / 2, y: 0, z: 0 },           // Vertical orbit
      { x: Math.PI / 3, y: Math.PI / 4, z: 0 }, // Tilted orbit 1
      { x: Math.PI / 6, y: -Math.PI / 4, z: 0 } // Tilted orbit 2
    ];
...
export function EcosystemOrbit3D({ brands }: EcosystemOrbit3DProps) {
